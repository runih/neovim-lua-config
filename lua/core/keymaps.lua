local fn_loaded, fn = pcall(require, "core.functions")
if not fn_loaded then
...
keymap.set("n", "<leader>so", fn.load_current_luafile, { desc = "[Lo]ading current lua file" })
keymap.set("n", "<leader>!", fn.execute_current_line, { desc = "Execute current line in to a buffer" })
...
if builtin_loaded then
  ...
  keymap.set("n", "|",                fn.find_in_current_buff,            { desc = "[|] Search in current buffer" })
  keymap.set("n", "<leader>sn",       fn.nvim_config,                     { desc = "[S]earch [N]eoVim configurations" })
end
...
if dap_loaded then
  ...
  keymap.set("n", "<leader>B",    fn.breakpoint_with_condition,                    { desc = "Toggle [B]reakpoint with condition"})
  keymap.set("n", "<leader>lp",   fn.breakpoint_based_on_log_point_message,        { desc = "Toggle breakpoint based on [l]og [p]oint message" })
...
if neotree_loaded then
  keymap.set("n", "<c-e>", fn.open_neotree, { desc = "Open N[E]oTree" })
end
